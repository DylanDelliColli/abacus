...
Transport is two-carriage per ADR-0003 (Accepted, revision 6): direct UDS where the sandbox permits, or a per-call host-approved `scribe-rpc` relay. Carriage selection is injected configuration with no fallback probing. Worker calls carry a launch-written non-secret Attempt locator and never caller-selected authority; Scribe resolves their binding from durable Assignment state. Decision verbs live on a separate authenticated orchestrator surface. The relay frame carries a bounded validated repo-id resolved against an injected runtime base.
...
- **Envelope** — the canonical sanitized context snapshot persisted against a launch subject — a worker Assignment/Attempt or a spawned orchestrator/watchdog actor activation (ADR-0003) — before the exact same content is delivered through Herdr.
- **Signal** — an immutable, idempotently appended coordination record with a closed type, a fenced sender that identifies the decision actor, and exactly one validated Bead, Assignment, Attempt, or responsibility-scope subject. Decision Signals retain the full exercised actor/profile/capability/scope snapshot. A worker Report retains the server-resolved actor/Assignment/Attempt binding; it deliberately does not invent capability or scope that Assignment state cannot reconstruct. No Signal accepts a subject-free body.
...
The operation is idempotent and supports a dry-run/plan mode suitable for automation. Initial actor provisioning is operator-only: Scribe's one-shot pre-listen bootstrap channel enrolls the first decision actor before the agent socket accepts any request (ADR-0003). If existing `.sable` or incompatible `.beads` state would be touched, initialization refuses with a migration/pilot diagnostic rather than reusing it.
...
3. Before any live delivery, Scribe records that sanitized Envelope snapshot with the Assignment/Attempt. Provider credentials and unrelated environment secrets are excluded from the Envelope entirely. ABACUS has no worker bearer credential or launch-secret sideband (ADR-0003).
4. Launch composition writes the non-secret Attempt locator into one reserved environment slot, then `abacus-runtime` starts a Herdr-managed agent with an explicit working directory/environment allowlist and delivers that exact Envelope. Runtime transports the locator opaquely; only Scribe resolves it.
...
7. On rejection, Scribe records the immutable Rejection, ends that Attempt, and leaves the Assignment active and the bead open/in progress. Only the Assignment's authorized decision actor may explicitly retry by appending a new fenced Attempt, possibly for the same worker. The retry commits the successor Attempt and its authorizing Retry decision atomically (`AttemptOpening`); the predecessor remains terminal and its fencing token is stale.
...
Lease expiry alone is insufficient because an old worker may resume. Every successor/reassigned Attempt receives a monotonically increasing fencing token; renewal extends the same Attempt's deadline without rotating its token or generation. Writes that carry an older token fail.
...
| Stale lease writer | Reject by fencing token before mutation; record no event for the refused non-operation |
...
| T4: live agent journey | One disposable repository and real Claude/Codex workers | Release candidate/manual diagnosis | Real providers and host approval policy |
