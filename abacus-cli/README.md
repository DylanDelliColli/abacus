...
**Transport note (ADR-0003, Accepted revision 6):** one versioned Scribe protocol has two injected carriages: direct UDS where permitted, or the facade binary's fixed two-token `scribe-rpc` relay. The relay is dispatched before ordinary argument parsing, project configuration, or repository discovery; it reads one bounded facade command envelope on stdin, privately frames one Scribe request, returns one bounded typed response, and exits. Carriage failure is loud and never triggers fallback probing. A worker envelope carries only the non-secret Attempt locator written into that launch, the current fencing token, operation identity, and payload. It contains no actor, Assignment, profile, capability, scope, credential, or other authority selector, and the strict decoder rejects such fields as unknown. Scribe alone resolves Attempt→Assignment→worker and admits only worker-trait verbs. Decision verbs use a separate authenticated orchestrator surface. The parallel fixed `runtime-rpc` composer accepts one bounded **narrow use-case** request and no execution material (no executable, argv, cwd, environment, or Envelope), loads persisted facts, and resolves the agent command solely from operator-owned allowlisted configuration before constructing `LaunchSpec`; accepting a client `LaunchSpec` would be arbitrary host execution. The exact host-approved argvs remain `/operator/path/abacus scribe-rpc` and `/operator/path/abacus runtime-rpc`. Operator enrolment tooling remains outside every agent-invocable surface.
