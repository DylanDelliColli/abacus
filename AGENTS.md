...
**Ceremony budget (the trap that killed the predecessor).** Design, review, and doctrine rounds are subject to the same law as any other work: each must gate a named deliverable, and "good enough to keep the build honest" is the bar. Meta-work is maximally seductive to capable agents — governance feels like the highest-leverage work available, reviews of governance feel rigorous, and both are infinitely extensible. **Judge a round by what it caught, not by its ratio to shipped code** (operator, 2026-08-06). Planning depth ahead of implementation is the deliberate answer to the predecessor's move-fast-then-patch-forever failure; a review that finds real defects is the cheapest work in this build, and a session that is mostly review is not evidence of drift. The rounds to kill are the ones that produce only refinement — no defect found, no decision resolved, no deliverable unblocked. **Two consecutive rounds like that, and you are the pane that needs the redirect.** When a design round exceeds roughly three exchanges without landing code *or finding anything*, freeze the design where it stands, record what it found as explicit debt on its bead, and go build the nearest real deliverable — an unresolved question that blocks nothing today is debt, not a blocker.

## Working agreement

- **Verify the seam before you draft.** Before designing against a module you did not write, read what its types actually carry and which verbs actually exist — then design against that, not against memory. Drafting first and letting review find the gap is the expensive order: ADR-0004 took seven revisions because each one was corrected by the code (records carrying no timestamp, a derived query excluding a whole Signal kind by design, an append verb that never existed, an identity needing a field no caller holds). All of it was readable in minutes beforehand. This is also where operator-level questions surface honestly — a fork you find in the seam at revision 1 is a decision; the same fork at revision 5 is a retraction.
