...
Output is an opaque, generation-fenced ABACUS runtime handle plus normalized launch facts. The adapter privately binds Herdr named-session namespace, pane ID, and terminal/process generation; callers receive none of those provider structures separately. Runtime does not render or mutate the Envelope; persistence and delivery therefore cannot drift.
