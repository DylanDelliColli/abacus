...
- the closed **launch subject** — a worker Assignment Attempt or a spawned orchestrator/watchdog actor activation, each carrying the credential (and, for activations, the generation) identity that prevents cross-subject material swaps (ADR-0003 R5.19). Every launch, recovery, and durable handle association is keyed by this same closed subject;
...
Output is an opaque, generation-fenced ABACUS runtime handle plus normalized launch facts. Launch additionally receives the subject's transient credential secret as a separate redacted sideband argument (never inside the launch specification, argv, environment, or Envelope; ADR-0003): the adapter delivers Envelope and secret as separate startup material without logging or child inheritance, and reports ONE closed startup-delivery outcome covering Envelope and secret together (`Submitted` = one provider API submission accepted, never proof of application; `NotDelivered(reason)` keeps the handle for stop/reconcile; `Ambiguous`) — an ambiguous delivery means the caller stops the session and then, for a worker subject, revokes the Attempt and opens a successor with fresh provisioning; for an actor activation, deactivates or rotates that activation — never a fictitious retry Attempt, and never a retry of the same secret. The adapter privately binds Herdr named-session namespace, pane ID, and terminal/process generation; callers receive none of those provider structures separately. Runtime does not render or mutate the Envelope; persistence and delivery therefore cannot drift.
...
The initial adapter targets Herdr's high-level CLI/JSON facade, not a hand-rolled socket client: named sessions plus `agent start`, `agent prompt`, `agent wait`, `agent read`, and bounded workspace/pane operations. The bundled API schema is still fingerprinted for diagnosis. Direct socket use is a later internal optimization only if evidence justifies it, with one explicit ADR-0003 exception that is **not** optional: **startup-material delivery speaks the pinned Herdr socket schema directly, host-side.** Source verification of pinned v0.8.0 shows `AgentStartParams` carries no material fields and the only high-level delivery verb (`agent prompt`) requires its text in argv, so the Envelope and the transient credential secret cannot ride the high-level CLI without argv exposure. Agents still never reach Herdr; secret ingress across the approval boundary is the facade's own fixed-dispatch `runtime-rpc` verb, whose public schema is a narrow use-case request carrying no execution material — the host side authenticates/authorizes `runtime:launch`, loads persisted Envelope and subject facts, resolves executable/args/env only from operator-owned allowlisted configuration, and only then constructs the internal `LaunchSpec`. A client-supplied `LaunchSpec` is never accepted. Compatibility evidence for this exact startup path is a blocking gate.

Each ABACUS repository uses a collision-resistant Herdr named-session namespace derived from its local repo ID. Herdr v0.7.5 does not relocate named-session state with config/socket overrides, so tests use an exact disposable namespace and pre/post manifest rather than claiming full path redirection. ABACUS never installs provider integrations/plugins or mutates the default session.
