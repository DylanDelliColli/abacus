...
| **Assignment** | The durable record binding one bead to one worker and naming its exact decision actor. An Assignment may contain several sequential Attempts; a failed, rejected, expired, revoked, or abort-compliant Attempt does not rewrite or implicitly terminate the Assignment. Created through Scribe. Assignments are ABACUS state, never stored in `br`. | Ledger |
| **Attempt** | One worker execution under an Assignment, carrying a Lease. Reclamation, rejection, expiry, revocation, or explicit worker compliance with a binding Abort ends that Attempt; an authorized decision actor may explicitly retry by appending a new one. Worker compliance records the distinct `Aborted` state rather than pretending the decision actor revoked it. Evidence and Handoffs bind to the Attempt that produced them. | Ledger |
...
| **Signal** | A durable, immutable, typed communication record: a fenced sender that identifies the decision actor plus a **required subject reference** — bead, assignment, attempt, or scope. Decision Signals carry the full exercised authority snapshot. A worker Report instead carries the Scribe-resolved worker/Assignment/Attempt binding; this deliberately weaker provenance does not fabricate capability or scope that Assignment state cannot reconstruct. Exactly three types: Directive, Report, Request. Signals carry no read/ack state; an unresolved Signal is a derived query (a Signal lacking its linked responding action), never an inbox flag (I19). Attention is a Herdr doorbell; content lives only in the Ledger. | Ledger |
